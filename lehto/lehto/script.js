...
function updateDates() {
    const dFI = document.getElementById('invoiceDate').value;
    const dISO = parseFI(dFI);

    document.getElementById('invoiceDateFI').textContent = formatDateFI(dISO);

    setDueDate(); // huolehtii nyt myös näytöstä
...
function setDueDate() {
    const invoiceFI = document.getElementById('invoiceDate').value;
    const invoiceISO = parseFI(invoiceFI);

    const days = getPaymentDays();
    const dueISO = addDaysISO(invoiceISO, days);

    // ✅ EI enää inputiin
    document.getElementById('dueDateFI').textContent = formatDateFI(dueISO);

    return dueISO; // palautetaan, jos muu koodi tarvitsee
}
